...
const getFontNameFromStyleAttrs = (node) => {
  ...
  return node.attrs && node.attrs.length > 0 ?
    node.attrs
...
const getFontNameFromStyleElement = (node) => {
  ...
  let fonts = node.childNodes[0].value.split(/[};]+/)
    .filter(style => style.includes('font-family'))
...
const getStylesheetFonts = style => {
  ...
  var result = style.match(/(?:#\w+\s+)?\.[\w-]+(?:\s+\w+\s*\.\w+|\s+\w+)?/ig)
  // result.map(str => {

  //   if (document.getElementsByClassName("banner-title")) {
  //     var node = document.getElementsByClassName("banner-title")[0];
  //   }
  // })
  return css.parse(style).stylesheet.rules
...
const filterStylesheets = html => {
  ...
  return parse5.parse(html).childNodes.find(node => node.nodeName === 'html').childNodes
